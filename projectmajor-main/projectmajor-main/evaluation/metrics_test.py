...
try:
    ...
    BaseMetricsTest = test_utils.BaseMetricsTest
except ImportError:
    # Stub for Windows
    class BaseMetricsTest(absltest.TestCase):
        def assertDictClose(self, result, expected, delta=1e-5):
            for k, v in expected.items():
                self.assertIn(k, result)
                self.assertAlmostEqual(result[k], v, delta=delta)


class MetricsTest(BaseMetricsTest):
